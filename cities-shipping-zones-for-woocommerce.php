			...
			if ( in_array( WC()->countries->get_base_country(), get_option( 'wc_csz_countries_codes' ) ) ) { ?>
				...
				jQuery( function( $ ) {
					...
					function billing_state_update() {
						...
						$( '#billing_new_state option' ).filter(
							function() {
								return $( this ).text() == $( '#billing_state option:selected' ).text().split( ' - ' )[0];
							})
							.prop( 'selected', true )
							.trigger( 'change' );
						var code = $( '#billing_state option:selected' ).val();
						$('#billing_postcode').val(code);
					}
					function shipping_state_update() {
						$( '#shipping_new_state option' ).filter( function() { return $( this ).text() == $( '#shipping_state option:selected' ).text().split( ' - ' )[0]; } ).prop( 'selected', true ).trigger( 'change' );
						var code = $( '#shipping_state option:selected' ).val();
						$('#shipping_postcode').val(code);
					}
				} );
				</script>
			<?php }
